...
describe("The bucky object", () => {
  ...
  test("Should have a property called 'nickName' with the value of 'Bucky' and not 'The Winter Soldier'", () => {
    expect(examples.bucky.nickname).not.toEqual("bucky");
    expect(examples.bucky.nickname).not.toEqual("The Winter Solider");
...
describe('The isAlphaOnly function', () => {
  ...
  test('should return true if string contains spaces and letters',
    () => {
  ...
  test('should return false if input is not a string',
    () => {
      expect(examples.isAlphaOnly(cap.calculateAge(6))).toBe(false);
      expect(examples.isAlphaOnly(cap.middleName)).toBe(false);
      expect(examples.isAlphaOnly(cap.catchphrases)).toBe(false);
      expect(examples.isAlphaOnly(cap)).toBe(false);
    }
  )
});
